...
TEST(FfiErrorCodesTest, ErrorsAreNonZero) {
    // Error codes are now positive (unified legends_error_t from ffi_core.h)
    EXPECT_NE(LEGENDS_ERR_INVALID_PARAM, 0);
    EXPECT_NE(LEGENDS_ERR_INVALID_HANDLE, 0);
    EXPECT_NE(LEGENDS_ERR_NOT_INITIALIZED, 0);
    EXPECT_NE(LEGENDS_ERR_ALREADY_INITIALIZED, 0);
    EXPECT_NE(LEGENDS_ERR_CPU, 0);
    EXPECT_NE(LEGENDS_ERR_DMA, 0);
    EXPECT_NE(LEGENDS_ERR_MEMORY, 0);
    EXPECT_NE(LEGENDS_ERR_CONFIG, 0);
    EXPECT_NE(LEGENDS_ERR_IO_PORT, 0);
    EXPECT_NE(LEGENDS_ERR_FATAL, 0);
...
TEST(FfiErrorCodesTest, ErrorCodesAreDistinct) {
    std::vector<int> codes = {
        LEGENDS_OK,
        LEGENDS_ERR_INVALID_PARAM,
        LEGENDS_ERR_INVALID_HANDLE,
        LEGENDS_ERR_NOT_INITIALIZED,
        LEGENDS_ERR_ALREADY_INITIALIZED,
        LEGENDS_ERR_CPU,
        LEGENDS_ERR_DMA,
        LEGENDS_ERR_MEMORY,
        LEGENDS_ERR_CONFIG,
...
TEST(FfiSafeCallTest, PassesThroughReturnCode) {
    ...
    EXPECT_EQ(result, LEGENDS_ERR_INVALID_PARAM);
}
...
TEST(FfiSafeCallTest, CatchesIllegalCpuStateException) {
    ...
    EXPECT_EQ(result, LEGENDS_ERR_CPU);
...
TEST(FfiSafeCallTest, CatchesDmaException) {
    ...
    EXPECT_EQ(result, LEGENDS_ERR_DMA);
...
TEST(FfiSafeCallTest, CatchesMemoryAccessException) {
    ...
    EXPECT_EQ(result, LEGENDS_ERR_MEMORY);
...
TEST(FfiSafeCallTest, CatchesConfigException) {
    ...
    EXPECT_EQ(result, LEGENDS_ERR_CONFIG);
...
TEST(FfiSafeCallTest, CatchesIoPortException) {
    ...
    EXPECT_EQ(result, LEGENDS_ERR_IO_PORT);
...
TEST(FfiSafeCallTest, CatchesFatalException) {
    ...
    EXPECT_EQ(result, LEGENDS_ERR_FATAL);
...
TEST(FfiSafeCallTest, CatchesCallbackException) {
    ...
    EXPECT_EQ(result, LEGENDS_ERR_INTERNAL);
...
TEST(FfiErrorCodeStringTest, OkReturnsOK) {
    EXPECT_STREQ(ffi::error_code_string(LEGENDS_OK), "OK");
}

TEST(FfiErrorCodeStringTest, AllCodesHaveStrings) {
    EXPECT_STRNE(ffi::error_code_string(LEGENDS_ERR_INVALID_PARAM), "Unknown error");
    EXPECT_STRNE(ffi::error_code_string(LEGENDS_ERR_INVALID_HANDLE), "Unknown error");
    EXPECT_STRNE(ffi::error_code_string(LEGENDS_ERR_NOT_INITIALIZED), "Unknown error");
    EXPECT_STRNE(ffi::error_code_string(LEGENDS_ERR_ALREADY_INITIALIZED), "Unknown error");
    EXPECT_STRNE(ffi::error_code_string(LEGENDS_ERR_CPU), "Unknown error");
    EXPECT_STRNE(ffi::error_code_string(LEGENDS_ERR_DMA), "Unknown error");
    EXPECT_STRNE(ffi::error_code_string(LEGENDS_ERR_MEMORY), "Unknown error");
    EXPECT_STRNE(ffi::error_code_string(LEGENDS_ERR_CONFIG), "Unknown error");
    EXPECT_STRNE(ffi::error_code_string(LEGENDS_ERR_IO_PORT), "Unknown error");
    EXPECT_STRNE(ffi::error_code_string(LEGENDS_ERR_FATAL), "Unknown error");
    EXPECT_STRNE(ffi::error_code_string(LEGENDS_ERR_INTERNAL), "Unknown error");
}
...
TEST(FfiCApiTest, AiboxLastErrorWorks) {
    ...
    EXPECT_STREQ(legends_ffi_last_error(), "C API test error");
}

TEST(FfiCApiTest, AiboxErrorTruncatedWorks) {
    ffi::clear_error();
    EXPECT_EQ(legends_ffi_error_truncated(), 0);

    std::string long_msg(3000, 'x');
    ffi::set_error(long_msg.c_str());
    EXPECT_EQ(legends_ffi_error_truncated(), 1);
}

TEST(FfiCApiTest, AiboxClearErrorWorks) {
    ffi::set_error("to be cleared");
    legends_ffi_clear_error();

    EXPECT_STREQ(legends_ffi_last_error(), "");
}

TEST(FfiCApiTest, AiboxErrorStringWorks) {
    EXPECT_STREQ(legends_ffi_error_string(LEGENDS_OK), "OK");
    EXPECT_STREQ(legends_ffi_error_string(LEGENDS_ERR_CPU), "CPU error");
    EXPECT_STREQ(legends_ffi_error_string(99999), "Unknown error");
}

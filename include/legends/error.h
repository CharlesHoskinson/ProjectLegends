 ...
 *
 * Usage:
 *   LEGENDS_CHECK(ptr != nullptr, ErrorCode::NullPointer, "pointer is null");
 */
#define LEGENDS_CHECK(cond, code, msg) \
    do { \
        if (!(cond)) { \
            return ::legends::make_error(code, msg); \
        } \
    } while (0)
 ...
 * @code
 *   Result<int> foo() {
 *       auto val = LEGENDS_TRY(bar());  // Returns if bar() fails
 *       return Ok(val + 1);
 *   }
 * @endcode
 *
 ...
 */
#define LEGENDS_TRY(expr) \
    ({ \
        auto&& _legends_result = (expr); \
        if (!_legends_result.has_value()) { \
            return ::legends::Err(_legends_result.error()); \
        } \
        std::move(_legends_result).value(); \
    })

/**
 * @brief Return early with error if condition is false.
 */
#define LEGENDS_ENSURE(cond, code, msg) \
    do { \
        if (!(cond)) { \
            return ::legends::make_error(code, msg); \
        } \
 ...
 */
#define LEGENDS_ERROR(code, msg) \
    ::legends::Error(code, msg, std::source_location::current())
...
#define LEGENDS_CURRENT_LOCATION std::source_location::current()

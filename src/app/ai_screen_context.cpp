...
// Two-call pattern: first call with nullptr queries the required buffer size,
// second call fills the cell buffer. This avoids guessing the screen dimensions.
std::string captureScreenContext(legends_handle handle, uint32_t max_chars) {
    ...
    legends_error_t err = legends_capture_text(handle, nullptr, 0, &cell_count, &info);
    ...
    err = legends_capture_text(handle, cells.data(), cell_count, &cell_count, &info);
    if (err != LEGENDS_OK) {
        return {};
    }

...
ExitCode Application::init(int argc, char** argv) {
    ...
    for (const auto& mount_arg : cli.mount_args) {
        ...
        if (ok && runtime_) {
            ...
            runtime_->mount_drive(parsed->letter,
    ...
    if (midi_config_.device != MIDIDevice::None && runtime_) {



        runtime_->set_midi_device(MIDIConfig::deviceName(midi_config_.device));
        ...
        if (!midi_config_.soundfont_path.empty()) {
            ...
            runtime_->set_midi_soundfont(midi_config_.soundfont_path);
        ...
        if (!midi_config_.mt32_romdir.empty()) {
            ...
            runtime_->set_midi_romdir(midi_config_.mt32_romdir);
...

    {
        ...
        if (!printer_dir.empty()) {
            ...
            if (printer_manager_.isEnabled() && runtime_) {



                runtime_->set_printer_output(printer_dir);
        ...
        if (!ttf_path.empty()) {
            ...
            if (ttf_renderer_.isEnabled() && runtime_) {



                runtime_->set_ttf_font(ttf_path, ttf_size);
    ...
    if (ipx_config_.enabled && runtime_) {



        runtime_->ipx_enable(true);
        ...
        if (!ipx_config_.server.empty()) {
            ...
            runtime_->ipx_connect(ipx_config_.server, ipx_config_.port);
    ...
    if (glide_config_.enabled && runtime_) {



        runtime_->glide_enable(true);



        runtime_->glide_set_resolution(glide_config_.width, glide_config_.height);
    ...
    if (pc98_config_.enabled && runtime_) {



        runtime_->set_machine_pc98(true);
    ...
    for (uint32_t i = 0; i < count; ++i) {
        ...
        switch (ev.type) {
            ...
            case pal::InputEventType::JoystickAxis: {
                ...
                runtime_->joystick_event(joy_id,
...
void Application::registerActionHandlers() {
    ...
    action_bus_.registerHandler(Action::Screenshot, [this](int) {
        ...
        runtime_->capture_rgb(nullptr, 0, &size_needed, &w, &h);
        ...
        runtime_->capture_rgb(buf.data(), buf.size(), &size_needed, &w, &h);
    ...
    action_bus_.registerHandler(Action::SaveState, [this](int slot) {
        ...
        if (size_needed > 0 && w > 0 && h > 0) {
            ...
            runtime_->capture_rgb(thumb.data(), thumb.size(), &size_needed, &w, &h);
    ...
    action_bus_.registerHandler(Action::AISubmitQuery, [this](int) {
        ...
        std::string screen = captureScreenContext(*runtime_, ai_config_.max_context_chars);
        ...
        runtime_->get_cursor(&cursor_x, &cursor_y, &cursor_visible);
        ...
        runtime_->capture_text(nullptr, 0, nullptr, &text_info);
    ...
    action_bus_.registerHandler(Action::SetMIDIDevice, [this](int param) {
        ...
        if (runtime_) {
            ...
            runtime_->set_midi_device(MIDIConfig::deviceName(device));
    ...
    action_bus_.registerHandler(Action::TogglePrinter, [this](int) {
        ...
        if (runtime_ && printer_manager_.isEnabled() && printer_manager_.isConfigured()) {



            runtime_->set_printer_output(printer_manager_.outputDirectory());
    ...
    action_bus_.registerHandler(Action::IPXConnect, [this](int) {
        ...
        if (runtime_ && !ipx_config_.server.empty()) {
    ...
    action_bus_.registerHandler(Action::ToggleGlide, [this](int) {
        ...
        if (runtime_) {
            ...
            runtime_->glide_enable(glide_config_.enabled);
    ...
    action_bus_.registerHandler(Action::SetMachinePC98, [this](int) {
        ...
        if (runtime_) {
            ...
            runtime_->set_machine_pc98(pc98_config_.enabled);

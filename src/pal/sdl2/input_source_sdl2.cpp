    ...
    bool translateEvent(const SDL_Event& sdl, InputEvent& event) {
        switch (sdl.type) {
            case SDL_KEYDOWN:
                ...
                event.key.scancode = legends::gsl::narrow<uint16_t>(sdl.key.keysym.scancode);
                event.key.keycode = legends::gsl::narrow_cast<uint16_t>(sdl.key.keysym.sym & 0xFFFF);
    ...
    bool translateWindowEvent(const SDL_WindowEvent& win, InputEvent& event) {
        switch (win.event) {
            ...
            case SDL_WINDOWEVENT_SIZE_CHANGED:
                ...
                event.window_resize.width = legends::gsl::narrow<uint32_t>(win.data1);
                event.window_resize.height = legends::gsl::narrow<uint32_t>(win.data2);
                return true;

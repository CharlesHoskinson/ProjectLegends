/**
 * @file test_dosboxx_abi.c
 * @brief Pure C ABI verification tests for dosboxx_embed.h
 *
 * This file MUST compile as pure C (C11) to verify the header
 * is usable from C code. Run with: gcc -std=c11 -c test_dosboxx_abi.c
 *
 * These tests verify:
...
static void test_abi_sizes(void) {
    ...
    assert(sizeof(dosboxx_text_cell_t) == 2);
    assert(sizeof(dosboxx_text_info_t) == 8);
    assert(sizeof(dosboxx_step_result_t) == 24);

    /* dosboxx_config_t size depends on pointer size and alignment.
    ...
    assert(sizeof(dosboxx_config_t) == 120);
#else
    /* 32-bit platforms: pointers are 4 bytes, no padding needed */
    assert(sizeof(dosboxx_config_t) == 108);
#endif
...
static void test_field_offsets(void) {
    ...
    /* dosboxx_text_cell_t */
    assert(offsetof(dosboxx_text_cell_t, character) == 0);
    assert(offsetof(dosboxx_text_cell_t, attribute) == 1);

    /* dosboxx_text_info_t */
    assert(offsetof(dosboxx_text_info_t, columns) == 0);
    assert(offsetof(dosboxx_text_info_t, rows) == 1);
    assert(offsetof(dosboxx_text_info_t, active_page) == 2);
    assert(offsetof(dosboxx_text_info_t, cursor_x) == 3);
    assert(offsetof(dosboxx_text_info_t, cursor_y) == 4);
    assert(offsetof(dosboxx_text_info_t, cursor_visible) == 5);

    /* dosboxx_step_result_t */
    assert(offsetof(dosboxx_step_result_t, cycles_executed) == 0);
    assert(offsetof(dosboxx_step_result_t, emu_time_us) == 8);
    assert(offsetof(dosboxx_step_result_t, stop_reason) == 16);
    assert(offsetof(dosboxx_step_result_t, events_processed) == 20);

    /* dosboxx_config_t - first fields should be at known offsets */
    assert(offsetof(dosboxx_config_t, struct_size) == 0);
    assert(offsetof(dosboxx_config_t, api_version) == 4);
    assert(offsetof(dosboxx_config_t, memory_kb) == 8);
    assert(offsetof(dosboxx_config_t, xms_kb) == 12);
    assert(offsetof(dosboxx_config_t, ems_kb) == 16);
    assert(offsetof(dosboxx_config_t, cpu_cycles) == 20);
    assert(offsetof(dosboxx_config_t, cpu_type) == 24);
    assert(offsetof(dosboxx_config_t, machine_type) == 28);
    assert(offsetof(dosboxx_config_t, deterministic) == 32);

    /* Pointer fields have platform-dependent offsets due to alignment */
#if defined(__LP64__) || defined(_WIN64) || defined(__x86_64__) || defined(__aarch64__)
    assert(offsetof(dosboxx_config_t, config_path) == 40);  /* 4 bytes padding after deterministic+_pad3 */
    assert(offsetof(dosboxx_config_t, working_dir) == 48);
    assert(offsetof(dosboxx_config_t, _reserved) == 56);
#else
    assert(offsetof(dosboxx_config_t, config_path) == 36);
    assert(offsetof(dosboxx_config_t, working_dir) == 40);
    assert(offsetof(dosboxx_config_t, _reserved) == 44);
#endif
...
static void test_version_constants(void) {
    ...
    assert(DOSBOXX_API_VERSION_MAJOR == 1);
    assert(DOSBOXX_API_VERSION_MINOR == 0);
    assert(DOSBOXX_API_VERSION_PATCH == 0);

    /* Verify packed version */
    uint32_t expected = (1 << 16) | (0 << 8) | 0;
    assert(DOSBOXX_API_VERSION == expected);

    printf("  Version constants verified: %u.%u.%u\n",
           DOSBOXX_API_VERSION_MAJOR,
           DOSBOXX_API_VERSION_MINOR,
...
static void test_error_codes(void) {
    ...
    /* DOSBOXX_OK must be 0 */
    assert(DOSBOXX_OK == 0);

    /* All error codes must be negative (or at least non-zero) */
    assert(DOSBOXX_ERR_NULL_HANDLE != 0);
    assert(DOSBOXX_ERR_NULL_POINTER != 0);
    assert(DOSBOXX_ERR_ALREADY_CREATED != 0);
    assert(DOSBOXX_ERR_NOT_INITIALIZED != 0);
    assert(DOSBOXX_ERR_ALREADY_RUNNING != 0);
    assert(DOSBOXX_ERR_BUFFER_TOO_SMALL != 0);
    assert(DOSBOXX_ERR_INVALID_CONFIG != 0);
    assert(DOSBOXX_ERR_INVALID_STATE != 0);
    assert(DOSBOXX_ERR_VERSION_MISMATCH != 0);
    assert(DOSBOXX_ERR_IO_FAILED != 0);
    assert(DOSBOXX_ERR_OUT_OF_MEMORY != 0);
    assert(DOSBOXX_ERR_NOT_SUPPORTED != 0);
    assert(DOSBOXX_ERR_INTERNAL != 0);

    /* All error codes must be distinct */
    dosboxx_error_t codes[] = {
        DOSBOXX_OK,
...
static void test_stop_reasons(void) {
    ...
    /* DOSBOXX_STOP_COMPLETED must be 0 */
    assert(DOSBOXX_STOP_COMPLETED == 0);

    /* All stop reasons must be distinct */
    assert(DOSBOXX_STOP_COMPLETED != DOSBOXX_STOP_HALT);
    assert(DOSBOXX_STOP_COMPLETED != DOSBOXX_STOP_BREAKPOINT);
    assert(DOSBOXX_STOP_COMPLETED != DOSBOXX_STOP_ERROR);
    assert(DOSBOXX_STOP_HALT != DOSBOXX_STOP_BREAKPOINT);
    assert(DOSBOXX_STOP_HALT != DOSBOXX_STOP_ERROR);
    assert(DOSBOXX_STOP_BREAKPOINT != DOSBOXX_STOP_ERROR);
...
static void test_config_initializer(void) {
    ...
    assert(config.struct_size == sizeof(dosboxx_config_t));
    assert(config.api_version == DOSBOXX_API_VERSION);
...
static void test_version_handshake(void) {
    ...
    err = dosboxx_get_api_version(&major, &minor, &patch);
    assert(err == DOSBOXX_OK);
    assert(major == DOSBOXX_API_VERSION_MAJOR);
    assert(minor == DOSBOXX_API_VERSION_MINOR);
    assert(patch == DOSBOXX_API_VERSION_PATCH);

    /* Null pointer handling */
    err = dosboxx_get_api_version(NULL, &minor, &patch);
    assert(err == DOSBOXX_ERR_NULL_POINTER);

    err = dosboxx_get_api_version(&major, NULL, &patch);
    assert(err == DOSBOXX_ERR_NULL_POINTER);

    err = dosboxx_get_api_version(&major, &minor, NULL);
    assert(err == DOSBOXX_ERR_NULL_POINTER);
...
static void test_instance_lifecycle(void) {
    ...
    dosboxx_handle handle = NULL;
    dosboxx_error_t err;

    /* Create with NULL config (defaults) */
    err = dosboxx_create(NULL, &handle);
    assert(err == DOSBOXX_OK);
    assert(handle != NULL);

    /* Second create should fail (single instance) */
    dosboxx_handle handle2 = NULL;
    err = dosboxx_create(NULL, &handle2);
    assert(err == DOSBOXX_ERR_ALREADY_CREATED);
    assert(handle2 == NULL);

    /* Destroy */
    err = dosboxx_destroy(handle);
    assert(err == DOSBOXX_OK);
...
static void test_null_handle_rejection(void) {
    ...
    /* Create null check */
    err = dosboxx_create(NULL, NULL);
    assert(err == DOSBOXX_ERR_NULL_POINTER);

    /* Functions that require handle */
    err = dosboxx_reset(NULL);
    assert(err == DOSBOXX_ERR_NULL_HANDLE);

    dosboxx_config_t config;
    err = dosboxx_get_config(NULL, &config);
    assert(err == DOSBOXX_ERR_NULL_HANDLE);

    err = dosboxx_step_ms(NULL, 100, NULL);
    assert(err == DOSBOXX_ERR_NULL_HANDLE);

    err = dosboxx_step_cycles(NULL, 1000, NULL);
    assert(err == DOSBOXX_ERR_NULL_HANDLE);

    /* Destroy null is OK (no-op) */
    err = dosboxx_destroy(NULL);
    assert(err == DOSBOXX_OK);
...
static void test_config_validation(void) {
    ...
    dosboxx_config_t bad_config = DOSBOXX_CONFIG_INIT;
    bad_config.struct_size = sizeof(dosboxx_config_t) - 1;  /* Wrong size */
    err = dosboxx_create(&bad_config, &handle);
    assert(err == DOSBOXX_ERR_INVALID_CONFIG);
    assert(handle == NULL);

    /* Wrong api_version should be rejected */
    bad_config = (dosboxx_config_t)DOSBOXX_CONFIG_INIT;
    bad_config.api_version = DOSBOXX_API_VERSION + 1;  /* Wrong version */
    err = dosboxx_create(&bad_config, &handle);
    assert(err == DOSBOXX_ERR_VERSION_MISMATCH);
    assert(handle == NULL);

    /* Valid config should work */
    dosboxx_config_t good_config = DOSBOXX_CONFIG_INIT;
    err = dosboxx_create(&good_config, &handle);
    assert(err == DOSBOXX_OK);
    assert(handle != NULL);
...
static void test_stepping_api(void) {
    ...
    dosboxx_config_t config;
    err = dosboxx_get_config(handle, &config);
    assert(err == DOSBOXX_OK);
...
static void test_frame_capture_api(void) {
    ...
    err = dosboxx_capture_text(handle, NULL, 0, &text_count, &info);
    assert(err == DOSBOXX_OK);
    ...
    dosboxx_text_cell_t* cells = (dosboxx_text_cell_t*)malloc(
        text_count * sizeof(dosboxx_text_cell_t));
    assert(cells != NULL);

    err = dosboxx_capture_text(handle, cells, text_count, &text_count, NULL);
    assert(err == DOSBOXX_OK);
    ...
    err = dosboxx_capture_rgb(handle, NULL, 0, &rgb_size, &width, &height);
    assert(err == DOSBOXX_OK);
    ...
    err = dosboxx_capture_rgb(handle, rgb_buffer, rgb_size, &rgb_size, NULL, NULL);
    assert(err == DOSBOXX_OK);
    ...
    int cursor_visible;
    err = dosboxx_get_cursor(handle, &cursor_x, &cursor_y, &cursor_visible);
    assert(err == DOSBOXX_OK);
...
static void test_input_api(void) {
    ...
    err = dosboxx_key_event(handle, 0x1E, 1);  /* Press */
    assert(err == DOSBOXX_OK);
    err = dosboxx_key_event(handle, 0x1E, 0);  /* Release */
    assert(err == DOSBOXX_OK);
    printf("    key_event: OK (press/release 'A')\n");

    /* Extended key event - Right Arrow (E0 + 0x4D) */
    err = dosboxx_key_event_ext(handle, 0x4D, 1);  /* Press */
    assert(err == DOSBOXX_OK);
    err = dosboxx_key_event_ext(handle, 0x4D, 0);  /* Release */
    assert(err == DOSBOXX_OK);
    printf("    key_event_ext: OK (Right Arrow)\n");

    /* Text input */
    err = dosboxx_text_input(handle, "Hello World");
    assert(err == DOSBOXX_OK);
    printf("    text_input: OK (\"Hello World\")\n");

    /* Text input with shift chars */
    err = dosboxx_text_input(handle, "ABC!@#");
    assert(err == DOSBOXX_OK);
    printf("    text_input: OK (shift chars)\n");

    /* Text input with newline */
    err = dosboxx_text_input(handle, "DIR\n");
    assert(err == DOSBOXX_OK);
    printf("    text_input: OK (newline)\n");

    /* Text input null rejection */
    err = dosboxx_text_input(handle, NULL);
    assert(err == DOSBOXX_ERR_NULL_POINTER);
    printf("    text_input: OK (rejects NULL)\n");

    /* Mouse event */
    err = dosboxx_mouse_event(handle, 10, 5, 0);  /* Move, no buttons */
    assert(err == DOSBOXX_OK);
    printf("    mouse_event: OK (move 10,5)\n");

    /* Mouse with buttons */
    err = dosboxx_mouse_event(handle, 0, 0, 1);  /* Left button */
    assert(err == DOSBOXX_OK);
    err = dosboxx_mouse_event(handle, 0, 0, 2);  /* Right button */
    assert(err == DOSBOXX_OK);
    err = dosboxx_mouse_event(handle, 0, 0, 4);  /* Middle button */
    assert(err == DOSBOXX_OK);
    printf("    mouse_event: OK (buttons)\n");

    /* Mouse negative movement */
    err = dosboxx_mouse_event(handle, -20, -15, 0);
    assert(err == DOSBOXX_OK);
    ...
    /* Verify input sets dirty flag */
    {
        ...
        size_t count;
        dosboxx_capture_text(handle, NULL, 0, &count, NULL);
        dosboxx_text_cell_t* cells = (dosboxx_text_cell_t*)malloc(
            count * sizeof(dosboxx_text_cell_t));
        dosboxx_capture_text(handle, cells, count, &count, NULL);
        free(cells);

        int dirty;
        dosboxx_is_frame_dirty(handle, &dirty);
        assert(dirty == 0);

        /* Key event should set dirty */
        dosboxx_key_event(handle, 0x1E, 1);
        dosboxx_is_frame_dirty(handle, &dirty);
        assert(dirty == 1);
    ...
    dosboxx_reset(handle);
    err = dosboxx_key_event(handle, 0x1E, 1);  /* Should still work */
    assert(err == DOSBOXX_OK);
...
static void test_save_state_api(void) {
    ...
    size_t state_size;
    err = dosboxx_save_state(handle, NULL, 0, &state_size);
    assert(err == DOSBOXX_OK);
    assert(state_size > 0);
    ...
    err = dosboxx_save_state(handle, buffer, state_size, &state_size);
    assert(err == DOSBOXX_OK);
    ...
    dosboxx_step_cycles(handle, 10000, NULL);
    ...
    /* Load saved state */
    err = dosboxx_load_state(handle, buffer, state_size);
    assert(err == DOSBOXX_OK);
    printf("    load_state: OK\n");

    /* Hash should match original after load */
    uint8_t hash3[32];
    err = dosboxx_get_state_hash(handle, hash3);
    assert(err == DOSBOXX_OK);
    ...
    /* Verify determinism */
    int is_det;
    err = dosboxx_verify_determinism(handle, 5000, &is_det);
    assert(err == DOSBOXX_OK);
    assert(is_det == 1);
    ...
    err = dosboxx_load_state(handle, invalid_buffer, sizeof(invalid_buffer));
    assert(err == DOSBOXX_ERR_INVALID_STATE);
...
static void test_log_callback_api(void) {
    ...
    /* Set log callback */
    err = dosboxx_set_log_callback(handle, test_log_callback, NULL);
    assert(err == DOSBOXX_OK);
    ...
    /* Clear callback */
    err = dosboxx_set_log_callback(handle, NULL, NULL);
    assert(err == DOSBOXX_OK);
...
static void test_error_message_api(void) {
    ...
    err = dosboxx_get_last_error(NULL, NULL, 0, &length);
    assert(err == DOSBOXX_OK);

    /* Trigger an error to have something to retrieve */
    dosboxx_handle handle = NULL;
    dosboxx_create(NULL, &handle);  /* Create one */
    dosboxx_handle handle2 = NULL;
    dosboxx_create(NULL, &handle2);  /* Second should fail */
    ...
    char buffer[256];
    err = dosboxx_get_last_error(NULL, buffer, sizeof(buffer), &length);
    assert(err == DOSBOXX_OK);
    assert(strlen(buffer) > 0);  /* Should have actual content */

    /* Buffer too small */
    char small_buffer[2];
    err = dosboxx_get_last_error(NULL, small_buffer, sizeof(small_buffer), &length);
    assert(err == DOSBOXX_ERR_BUFFER_TOO_SMALL);

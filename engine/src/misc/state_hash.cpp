...
Result<StateHash> get_state_hash(DOSBoxContext* ctx, HashMode mode) {
    ...
    // ─────────────────────────────────────────────────────────────────────
    ctx->dos.hash_into(builder);

    // Placeholder marker for state not yet migrated
    const char* placeholder = "STATE_HASH_V9";
...
// Implementation moved to state_hash_compat.cpp to isolate current_context() usage
// per Sprint 2 policy. See get_state_hash(HashMode mode) in state_hash_compat.cpp
// ─────────────────────────────────────────────────────────────────────────────

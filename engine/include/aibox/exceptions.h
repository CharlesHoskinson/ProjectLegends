...
/**
 * @brief Base class for all AIBox/DOSBox emulator exceptions.
 *
 ...
 *
 * Usage: Replace `abort()` with `AIBOX_ABORT("reason")`
 */
#ifdef AIBOX_LIBRARY_MODE
    #define AIBOX_ABORT(msg) \
        throw ::aibox::FatalException(msg, __FILE__, __LINE__)
#else
    #include <cstdlib>
    #include <iostream>
    #define AIBOX_ABORT(msg) \
 ...
 */
#ifdef AIBOX_LIBRARY_MODE
    #define AIBOX_ASSERT(cond, msg) \
        do { \
            if (!(cond)) { \
                throw ::aibox::FatalException( \
                    std::string("Assertion failed: ") + (msg), __FILE__, __LINE__); \
            } \
        } while(0)
#else
    #include <cassert>
    #define AIBOX_ASSERT(cond, msg) assert((cond) && (msg))
#endif

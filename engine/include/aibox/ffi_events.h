...
#ifndef AIBOX_FFI_EVENTS_H
#define AIBOX_FFI_EVENTS_H
 ...
 */
typedef enum {
    AIBOX_EVENT_TEXT_SCREEN = 1,       /**< Full text mode screen */
    AIBOX_EVENT_TEXT_DIFF = 2,         /**< Differential text update */
    AIBOX_EVENT_PROGRAM_STARTED = 3,   /**< Program started */
    AIBOX_EVENT_PROGRAM_HALTED = 4,    /**< Program terminated */
    AIBOX_EVENT_KEYBOARD = 5,          /**< Keyboard input */
    AIBOX_EVENT_MOUSE = 6,             /**< Mouse input */
    AIBOX_EVENT_LOG = 7                /**< Log message */
} aibox_event_type_t;
...
typedef struct {
    ...
    const aibox_text_cell_t* cells;  /**< Cell array [rows * columns] */
} aibox_text_screen_event_t;
...
typedef struct {
    ...
    aibox_text_cell_t cell;    /**< New cell value */
} aibox_text_change_t;
...
typedef struct {
    ...
    const aibox_text_change_t* changes;  /**< Array of changes */
} aibox_text_diff_event_t;
 ...
 */
typedef enum {
    AIBOX_TERM_NORMAL_EXIT = 0,    /**< INT 21h AH=4Ch */
    AIBOX_TERM_CTRL_BREAK = 1,     /**< Ctrl+Break pressed */
    AIBOX_TERM_CRITICAL_ERROR = 2, /**< DOS critical error */
    AIBOX_TERM_EXCEPTION = 3,      /**< CPU exception */
    AIBOX_TERM_PARENT_KILL = 4,    /**< Parent terminated child */
    AIBOX_TERM_UNKNOWN = 5         /**< Unknown reason */
} aibox_termination_reason_t;
...
typedef struct {
    uint64_t timestamp_us;                    /**< Event timestamp */
    aibox_termination_reason_t reason;        /**< Termination reason */
...
typedef enum {
    AIBOX_MOUSE_NONE   = 0,
    AIBOX_MOUSE_LEFT   = 1 << 0,
    AIBOX_MOUSE_RIGHT  = 1 << 1,
    AIBOX_MOUSE_MIDDLE = 1 << 2
} aibox_mouse_buttons_t;
 ...
 *
 * @param event_type Type of event (aibox_event_type_t)
 * @param event_data Pointer to event structure (type depends on event_type)
 * @param user_data User-provided context
 */
typedef void (*aibox_event_callback_t)(
    aibox_event_type_t event_type,
 ...
 */
typedef void (*aibox_text_screen_callback_t)(
    const aibox_text_screen_event_t* event,
    void* user_data
);
 ...
 */
typedef void (*aibox_text_diff_callback_t)(
    const aibox_text_diff_event_t* event,
    void* user_data
);
 ...
 */
typedef void (*aibox_program_started_callback_t)(
    const aibox_program_started_event_t* event,
    void* user_data
);
 ...
 */
typedef void (*aibox_program_halted_callback_t)(
    const aibox_program_halted_event_t* event,
    void* user_data
);
 ...
 */
typedef void (*aibox_keyboard_callback_t)(
    const aibox_keyboard_event_t* event,
    void* user_data
);
 ...
 */
typedef void (*aibox_mouse_callback_t)(
    const aibox_mouse_event_t* event,
    void* user_data
);
 ...
 */
aibox_subscription_t aibox_subscribe(
    aibox_handle_t handle,
    aibox_event_type_t event_type,
    aibox_event_callback_t callback,
    void* user_data
);
 ...
 *
 * @param subscription Subscription handle from aibox_subscribe
 */
void aibox_unsubscribe(aibox_subscription_t subscription);
 ...
 */
aibox_error_t aibox_enable_text_diff(aibox_handle_t handle, int enable);
 ...
 */
int32_t aibox_flush_events(aibox_handle_t handle);
 ...
 */
int32_t aibox_pending_event_count(aibox_handle_t handle);
 ...
 *
 * @param struct_name Name of structure (e.g., "aibox_text_cell_t")
 * @return Size of structure in bytes, or 0 if unknown
 */
size_t aibox_get_struct_size(const char* struct_name);
...
#ifdef __cplusplus
static_assert(sizeof(aibox_text_cell_t) == 4,
              "aibox_text_cell_t must be 4 bytes");
static_assert(sizeof(aibox_keyboard_event_t) == 12,
              "aibox_keyboard_event_t must be 12 bytes");
static_assert(sizeof(aibox_mouse_event_t) == 16,
              "aibox_mouse_event_t must be 16 bytes");
#endif

#endif /* AIBOX_FFI_EVENTS_H */

...
/* E_Exit: In library mode, throws exception instead of terminating.
 * The noreturn attribute is removed in library mode since it may return
 * via exception propagation to the API boundary.
 */
#ifdef DOSBOX_LIBRARY_MODE
void E_Exit(const char * format,...) GCC_ATTRIBUTE( __format__(__printf__, 1, 2));
#else
...
protected:
    ...
    {
        for (auto& pod : podRef)
            stream.write(static_cast<const char*>(pod.address), pod.size);
    }

    void setBytes(std::istream& stream) override
    {
        for (auto& pod : podRef)
            stream.read(static_cast<char*>(pod.address), pod.size);
    }
